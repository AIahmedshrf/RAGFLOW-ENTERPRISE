docker network inspect docker_ragflow --format '{{json .Containers}}' | jq .
docker compose down --remove-orphans
docker network rm docker_ragflow 2>/dev/null || true
 docker rm -f docker-ragflow-cpu-1 docker-es01-1 2>/dev/null || true

 1) إيقاف كل خدمات الـ Docker بالكامل
 cd /srv/projects/RAGFLOW-ENTERPRISE/docker
# إن قال إن الشبكة لا تزال «in use»، اعرف من المتصل بها:
docker network inspect docker_ragflow --format '{{json .Containers}}' | jq .

# أوقف خدمات الـ compose + الأيتام
docker compose down --remove-orphans

# أوقف/احذف حاوية TEI المستقلة إن وُجدت
docker rm -f tei 2>/dev/null || true

docker rm -f tei-bge 2>/dev/null || true

# (اختياري عند تعلّق الشبكة) إزالة الشبكة وإعادة إنشائها تلقائياً لاحقاً
docker network rm docker_ragflow 2>/dev/null || true

docker rm -f docker-ragflow-cpu-1 docker-es01-1 2>/dev/null || true


# تحقق من صحة التكوين قبل الإقلاع
docker compose config >/dev/null && echo "OK: compose config valid"

2) تشغيل كل الخدمات بالكامل
cd /srv/projects/RAGFLOW-ENTERPRISE/docker

# شغّل البنية التحتية و RAGFlow (CPU)
docker compose --profile cpu up -d mysql redis minio es01 ragflow-cpu

# شغّل TEI على نفس الشبكة باستخدام النماذج المحلية
docker run -d --name tei \
  --network docker_ragflow \
  -v /srv/models/hf:/data \
  ...
  --pooling mean
# للتبديل إلى BGE-M3 لاحقاً:
# docker restart tei && docker exec tei pkill -f text-embeddings || true
# docker rm -f tei && docker run -d --name tei --network docker_ragflow -v /srv/models/hf:/data \
#   -e HUGGINGFACE_HUB_CACHE=/data --restart unless-stopped \
#   ghcr.io/huggingface/text-embeddings-inference:cpu-1.7.2 \
#   --model-id /data/bge-m3 --pooling mean


شغّل خادمي TEI (واحد لـ e5-large وآخر لـ bge-m3)
# 1) e5-large
docker run -d --name tei-e5 \
  --network docker_ragflow \
  -v /srv/models/hf:/data \
  -e HUGGINGFACE_HUB_CACHE=/data \
  --restart unless-stopped \
